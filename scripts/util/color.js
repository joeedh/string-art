import {util, math, Vector3, Vector4} from '../path.ux/pathux.js';
...
let hsv_to_rgb_rets = util.cachering.fromConstructor(Vector3, 64);

export function hsv_to_rgb(h, s, v) {
  let c = 0, m = 0, x = 0;
  let color = hsv_to_rgb_rets.next();
  ...
  if (h >= 0.0 && h < 60.0) {
    color.loadXYZ(c + m, x + m, m);
  } else if (h >= 60.0 && h < 120.0) {
    color.loadXYZ(x + m, c + m, m);
  } else if (h >= 120.0 && h < 180.0) {
    color.loadXYZ(m, c + m, x + m);
  } else if (h >= 180.0 && h < 240.0) {
    color.loadXYZ(m, x + m, c + m);
  } else if (h >= 240.0 && h < 300.0) {
    color.loadXYZ(x + m, m, c + m);
  } else if (h >= 300.0 && h < 360.0) {
    color.loadXYZ(c + m, m, x + m);
  } else {
    color.loadXYZ(m, m, m);
  }
...
export class ImageHistogram {
  ...
  getMedianColor() {
    ...
    for (let i = 0; i < this.n; i++) {
      if (nmax === undefined | this.l[i] > nmax) {
        nmax = this.l[i];
        lmax = (i/this.n)//*2.0;
      }
    }
    ...
    nmax = undefined;
    for (let i = 0; i < this.n; i++) {
      if (nmax === undefined | this.a[i] > nmax) {
        nmax = this.a[i];
        amax = (i/this.n)//*2.0 - 1.0;
      }
    }
    ...
    nmax = undefined;
    for (let i = 0; i < this.n; i++) {
      if (nmax === undefined | this.b[i] > nmax) {
        nmax = this.b[i];
        bmax = (i/this.n)//*2.0 - 1.0;
      }
    }

    console.log(lmax, amax, bmax);
    return [lmax, amax, bmax];
    return lab_to_rgb(lmax, amax, bmax);
  }

  calc(image, weightByCentDist = false) {
    const idata = image.data;
    const idiv = 1.0/255.0;
    const n = this.n;

    for (let i = 0; i < idata.length; i += 4) {
      let r = idata[i]*idiv, g = idata[i + 1]*idiv, b = idata[i + 2]*idiv, a = idata[i + 3]*idiv;
      let w = 1.0;

      if (weightByCentDist) {
        let idx = i/4;
        let x = (idx%image.width)/image.width;
        let y = (~~(idx/image.width))/image.height;
      ...
      if (i%5000 === 0) {
        console.log(lab[0].toFixed(3), lab[1].toFixed(3), lab[2].toFixed(3));
        console.log(li, ai, bi);
      }
      if (li >= this.n || ai >= n || bi >= n || li < 0 || ai < 0 || bi < 0) {
        debugger;
      }
